...
@push('title')
    Perpus App | Master Pengarang
@endpush

@section('content')
<div class="p-4 pb-12 pt-20 sm:ml-72">
    <div class="my-4 text-center">
        <h2 class="text-xl font-extrabold leading-none tracking-tight text-gray-900 md:text-3xl lg:text-4xl dark:text-white">Master Pengarang <span class="bg-blue-100 text-blue-800 text-2xl font-semibold mr-2 px-2.5 rounded dark:bg-blue-200 dark:text-blue-800 ml-2">Beta</span></h2>
            ...
            <div class="relative w-full max-w-md max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="px-6 py-6 lg:px-8">
                        <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Tambah Data Pengarang</h3>
                        <form id="createForm" class="space-y-6" action="#" enctype="multipart/form-data"
                            onsubmit="event.preventDefault(); createPengarang();"
                        >
                            @csrf
                            <div>
                                <label for="nama_pengarang" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Pengarang</label>
                                <div class="flex">
        ...
        <div id="editModal" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative w-full max-w-md max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="px-6 py-6 lg:px-8">
                        <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Edit Data Pengarang</h3>
                        <form class="space-y-6" action="#">
                            <div>
                                <label for="nis" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">NIS (Nomor Induk Pengarang)</label>
                            ...
                            <div>
                                <label for="nama" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Pengarang</label>
...
<script>
    ...
    function fetchDataAndAppendToTable(params, pageUrl = "{{ route('dashboard.master-pengarang.getData') }}") {
        // Make a GET request using Axios with the provided params
        // axios.get("{{ route('dashboard.master-pengarang.getData') }}", {
        //     params: params
        // })
        axios.get(pageUrl, {
            params: params
            ...
            .then(function (response) {
                ...
                `
                    <tr class="bg-white text-center border-b dark:bg-gray-800 dark:border-gray-700">
                        <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">${item.nama_pengarang}</td>
    ...
    function handleDeleteAction(pengarangId) {
        resetModalPosition();
        ...
        confirmButton.addEventListener('click', () => {
            ...
            axios
                .post(`{{ route('dashboard.master-pengarang.delete') }}`, {
                    _method: 'DELETE',
                    id: pengarangId
                })
                .then(response => {
                    // Handle the response (e.g., show a success message)
                    console.log('Pengarang deleted successfully', response);
                    fetchDataAndAppendToTable(); // Refresh the table after deletion
                })
                .catch(error => {
                    // Handle any errors (e.g., show an error message)
                    console.error('Error deleting Pengarang', error);
                });
    ...
    function createPengarang() {
        const form = document.getElementById('createForm');
        const formData = new FormData(form);


        axios.post("{{ route('dashboard.master-pengarang.create') }}", formData)
            .then(response => {
                // Handle the response (e.g., show a success message)
                console.log('Pengarang created successfully', response);
                fetchDataAndAppendToTable(); // Refresh the table after deletion

            })
            .catch(error => {
                // Handle any errors (e.g., show an error message)
                console.error('Error creating Pengarang', error);
            });
